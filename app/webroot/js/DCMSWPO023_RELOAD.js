...
function initReload() {
	...
	if (get.length != 0) {
		...
		if (get["batch_nm_cd"] != null) {
			$("WPO023ModelBatchNmCd").setValue(get["batch_nm_cd"]);
		}

		if (get["s_batch_no"] != null) {
			$("WPO023ModelSBatchNo").setValue(decodeURI(get["s_batch_no"]));
		}

		if (get["s_kanri_no"] != null) {
			$("WPO023ModelSKanriNo").setValue(decodeURI(get["s_kanri_no"]));
		}

		if (get["zone"] != null) {

			$("WPO023ModelZone").setValue(get["zone"]);
		}

		if (get["staff_nm"] != null) {
			$("WPO023ModelStaffNm").setValue(decodeURI(get["staff_nm"]));
		}

		if (get["kbn_status"] != null) {
			$("WPO023ModelKbnStatus").setValue(get["kbn_status"]);
		}
...
function reloading() {
	...
	location.href="/DCMS/WPO023/index?" + prams;
}
...
function clickReloadButton(event) {
	...
	if ($("koshinButton").hasClassName("selected") ) {
		...
		location.href="/DCMS/WPO023/update?" + prams;
