...
function initReload() {
	...
	if (get.length != 0) {
		...
		if (get["bnri_dai_cd"] != null) {
			$("WEX030ModelBnriDaiCd").setValue(get["bnri_dai_cd"]);
		}

		if (get["bnri_dai_nm"] != null) {
			$("WEX030ModelBnriDaiNm").setValue(decodeURI(get["bnri_dai_nm"]));
		}

		if (get["bnri_dai_exp"] != null) {
			$("WEX030ModelBnriDaiExp").setValue(decodeURI(get["bnri_dai_exp"]));
		}
...
function clickUploadButton(event) {
    ...
    new Ajax.Request( actionUrl3, {
	  ...
	  onFailure : function( event )  {
	    location.href = "/DCMS/WEX030/index?return_cd=91&message=" + Base64.encode(DCMSMessage.format("CMNE000103"));
	  },
	  onException : function( event, ex )  {
	    location.href = "/DCMS/WEX030/index?return_cd=92&message=" + Base64.encode(DCMSMessage.format("CMNE000104"));
	  }
	});
}
...
function onSuccessEvent(res) {
	...
	if (get.length != 0) {
		...
		if (get["bnri_dai_cd"] != null) {
			bnri_dai_cd = get["bnri_dai_cd"];
		}

		if (get["bnri_dai_nm"] != null) {
			bnri_dai_nm = decodeURI(get["bnri_dai_nm"]);
		}

		if (get["bnri_dai_exp"] != null) {
			bnri_dai_exp = decodeURI(get["bnri_dai_exp"]);
		}
	...
	location.href = "/DCMS/WEX030/index?" + result + "&bnri_dai_cd=" + bnri_dai_cd +
