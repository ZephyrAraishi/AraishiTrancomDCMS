...
AbstractProtoCalendarRender.prototype = {
  ...
  createContainer: function() {
    ...
    if (this.options.onlyMonth == true) {
  ...
  ProtoCalendarRender.prototype,
  {
    ...
    renderBody: function(calendar) {
      ...
      if (this.options.onlyMonth == true) {
	      ...
	      html = '<table class="#{tableClass}" cellspacing="0">';
	      ...
	      for(var i = 0; i < dayNum; i += 1, curDay.setDate(curDay.getDate() + 1)) {
	        ...
	        if (this.isSelectable(curDay)) {
	          ...
	          dayHtml = '<a class="#{dayClass}" href="#" id="' + dayId +
	            ...
	            '" day="' + curDay.getDate() +
	            '" style="color:black;background:white;">決定</a>';
...
var InputMonthCalendar = Class.create();
InputMonthCalendar.createOnLoaded = function(input, options) {
  BaseCalendar.bindOnLoad(function() {
    new InputMonthCalendar(input, options);
  });
};
InputMonthCalendar.initCalendars = function(inputs, options) {
  if (document.observe) {
    document.observe('dom:loaded', function() {
      $$(inputs).each(function(input) {
        new InputMonthCalendar(input, options);
...
Object.extend(InputMonthCalendar.prototype, BaseCalendar.prototype);
Object.extend(
  InputMonthCalendar.prototype,
  {
    initialize: function(input, options) {
      this.input = $(input); // used in doObserveEvents()
      this.initializeOptions(options);
