...
function clickUploadButton(event) {
    ...
    new Ajax.Request( actionUrl3, {
	  ...
	  onFailure : function( event )  {
	    location.href = "/DCMS/WEX080/index?return_cd=91&message=" + Base64.encode(DCMSMessage.format("CMNE000103"));
	  },
	  onException : function( event, ex )  {
	    location.href = "/DCMS/WEX080/index?return_cd=92&message=" + Base64.encode(DCMSMessage.format("CMNE000104"));
	  }
	});
}
...
function onSuccessEvent(res) {
	...
	if(result != "return_cd=0") {
		...
		//ロケーションの変更
		location.href = "/DCMS/WEX080/index?" + result + "&ymd_sime=" + ymd_sime +
														 "&display=true";
	} else {
...
function onSuccessEvent2(res) {
	...
	if (return_cd == "1") {
		...
		if(!window.confirm(Base64.decode(message))){
			...
			location.href = "/DCMS/WEX080/index?&ymd_sime=" + ymd_sime;
			return;
		}
...
function onSuccessEvent3(res) {
	...
	if (return_cd == "1") {
		...
		return;
	} else if(result == "return_cd=0_1") {
		
		if(!window.confirm(DCMSMessage.format("CMNE000025"))){
...
function onSuccessEvent4(res) {
	...
	//ロケーションの変更
	location.href = "/DCMS/WEX080/index?" + result + "&ymd_sime=" + ymd_sime + "&display=true";
