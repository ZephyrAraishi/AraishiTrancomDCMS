...
function clickPopUp(event) {
	...
	new Ajax.Request( actionUrl, {
	  ...
	  onFailure : function( event )  {
	    location.href = "/DCMS/WPO023/index?return_cd=91&message=" + Base64.encode(DCMSMessage.format("CMNE000101"));
	  },
	  onException : function( event, ex )  {
	    location.href = "/DCMS/WPO023/index?return_cd=92&message=" + Base64.encode(DCMSMessage.format("CMNE000102"));
	  }
	});
}
...
function viewPopUp(event) {
    ...
    var kbnStatu = data[5].innerHTML;
	//var options = $$('select#kbnStatus option');
...
function clickOKButton(window) {
	...
	var options = $A($('kbnStatus').getElementsByTagName('option'));
	options.each(function(option){
	
		if (option.value == kbnStatus) {
			kbnStatusNm = option.innerHTML;
		}
		
	});

	if (kbnStatus != originValue) {
	
		setUpdatedCellColor(cells[6]);
		
		cells[6].innerHTML = kbnStatusNm;
		data[4].update(kbnStatusNm);
		data[5].update(kbnStatus);
		data[8].update("1");
