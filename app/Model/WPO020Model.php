...
class WPO020Model extends DCMSAppModel{
	
	public $name = 'WPO020Model';
	...
	public function getSKanriLst($batch_nm_cd, $s_batch_no, $s_kanri_no, $zone, $staff_nm, $kbn_status) {
		...
		try {
			...
			$this->printLog("fatal", "例外発生", "WPO020", $e->getMessage());
		}
	 ...
	 * @return   成否情報(true,false)
	 */
	public function setTANAKETSU($staff_cd,$arrayList) {
		...
		try{
			...
			$this->execWithPDOLog($pdo,$sql,'WPO020 排他制御オン');
			$pdo = null;
			...
			try {
				...
				foreach($arrayList as $array) {
					...
					$sql = "CALL P_WPO020_SET_TANAKETSU(";
					$sql .= "'" . $staff_cd . "',";
					$sql .= "'" . $this->_ninusi_cd . "',";
					$sql .= "'" . $this->_sosiki_cd . "',";
					$sql .= "'" . $S_BATCH_NO_CD . "',";
			...
			$this->execWithPDOLog($pdo,$sql,'WPO020 排他制御オフ');
			$pdo = null;
