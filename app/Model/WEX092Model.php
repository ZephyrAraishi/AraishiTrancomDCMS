...
class WEX092Model extends DCMSAppModel{
	
	public $name = 'WEX092Model';
	...
	*/
	 ...
	 * @param $startYm 対象期間FROM
	 * @param $endYm 対象期間TO
	 * @return ABCデータベース月別情報取得
	 */
	function getAbcMonthlyData($bnriDaiCd, $bnriCyuCd, $bnriSaiCd, $startYm, $endYm) {
		...
		$conditionVal['start_ym'] = $startYm;
		$conditionVal['end_ym'] = $endYm;
			
		// 発行するSQLを設定
		$sql  = "select ";
		$sql .= "$fields ";
		$sql .= "from ";
		$sql .= "	V_WEX092_T_ABC_DB_MONTH_LST ";
		$sql .= "where ";
		$sql .= "    NINUSI_CD = :ninusi_cd ";
		$sql .= "and SOSIKI_CD = :sosiki_cd ";
		if (!empty($bnriDaiCd)) {
		...
		$sql .= "and YM between :start_ym and :end_ym ";
		$sql .= "order by ";
		$sql .= "    YM desc ";
	
		// 改善データ情報を取得
		$data = $this->queryWithLog($sql, $conditionVal, "ABCデータベース月別情報");
