 ...
 * @author        ZephyrLabo
 * @version       1.1
 * @memo          2017/09/29 K.Araishi WPO080を元に新規作成
 */

App::uses('MMessage', 'Model');
App::uses('MCommon', 'Model');

class WPO090Model extends DCMSAppModel{

	public $name = 'WPO090Model';
	...
	public function loadIfShukka() {
		...
		$data = $this->queryWithLog($sql, "", "WPO090 IF_追加出荷指示データ取得");
		return $data;
	}
	...
	public function checkIfShukka() {
		...
		try{
			...
			$data = $this->queryWithLog($sql, "", "WPO090 IF_追加出荷指示データエラーチェック");

			if ($data[0][0]['A1'] != 0 or !empty($data[0][0]['A1'])) {
				$this->errors['Check'][] = "取り込んだ追加出荷指示データにエラーがある為、取り込めません。";
				return false;
			}
			...
			$data = $this->queryWithLog($sql, "", "WPO090 IF_追加出荷指示データ商品エラーチェック");
			...
			if ($data[0][0]['A1'] != 0) {
				...
				$sql  .= "   and sh.shohin_code is null ";
				$sql  .= ") as tmp)";
				$this->execWithPDOLog($pdo, $sql, 'WPO090 IF_出荷指示データチェック内容更新');

				return false;
			}

			// IF出荷指示データのSS_NUMBER,SS_NUMBER_LINEで既に出荷指示が取り込まれていたら取り込まない
			$sql  = "select count(*) as A1 from t_ozax_shukka";
			$sql  .= " where NINUSI_CD = '" . $this->_ninusi_cd . "'";
			$sql  .= "   and SOSIKI_CD = '" . $this->_sosiki_cd . "'";
			$sql  .= "   and (SS_NUMBER,SS_NUMBER_LINE,SHUKKA_BI) in (select SS_NUMBER,SS_NUMBER_LINE,SHUKKA_BI from if_ozax_shukka";
			$sql  .= "        where NINUSI_CD = '" . $this->_ninusi_cd . "'";
			$sql  .= "          and SOSIKI_CD = '" . $this->_sosiki_cd . "')";
			$data = $this->queryWithLog($sql, "", "WPO090 IF_出荷指示データSS_NUMBERエラーチェック");

			if ($data[0][0]['A1'] != 0) {
				$this->errors['Check'][] = "取り込んだ出荷指示データはSS_NUMBER、SS_NUMBER_LINEで重複しています。";
				return false;
			}
			...
			$pdo = null;
			$this->printLog("info", "例外発生", "WPO090", $e->getMessage());
	...
	public function loadCheckIfShukka() {
		...
		$sql  .= " where NINUSI_CD = '" . $this->_ninusi_cd . "'";
		$sql  .= " and SOSIKI_CD = '" . $this->_sosiki_cd . "'";
		$data = $this->queryWithLog($sql, "", "WPO090 IF_出荷指示データ件数取得");
		return $data;
	}
	...
	public function InsertIfShukka($records, $staffCd) {
		...
		try{
			...
			$sql  .= " where NINUSI_CD = '" . $this->_ninusi_cd . "'";
			$sql  .= " and SOSIKI_CD = '" . $this->_sosiki_cd . "'";
			$this->execWithPDOLog($pdo, $sql, 'WPO090 IF_出荷指示データ削除');
			...
			foreach ( $records as $line ) {
				...
				$this->printLog("info", "IF出荷取込", "WPO090", $sql);
				$this->execWithPDOLog($pdo, $sql, 'WPO090 IF_出荷指示データ取込');
			}
...
//			$message .= "店舗名が空白です。";
	...
	public function updateShukka($staffCd) {
		...
		try{
			...
			while($result_henkan = $stmt_henkan->fetch(PDO::FETCH_ASSOC)){
				...
				// 出荷指示データ変換クエリー実行
				$sql .= $where;
				$this->execWithPDOLog($pdo, $sql, 'WPO090 出荷指示データ変換クエリー実行');
			...
			$sql .= " WHERE NINUSI_CD = '" . $this->_ninusi_cd . "'";
			$sql .= "   AND SOSIKI_CD = '" . $this->_sosiki_cd . "'";
			$this->execWithPDOLog($pdo, $sql, 'WPO090  出荷指示データ作成');
			...
			$sql .= ",SYOMI_KIGEN";
			$sql .= ";";
			$this->execWithPDOLog($pdo, $sql, 'WPO090 トータルピッキングデータ作成');
			...
			while($result_totalpk = $stmt_totalpk->fetch(PDO::FETCH_ASSOC)){
				...
				$sql .= ",@renban";
				$sql .= ")";
				$this->execWithPDOLog($pdo, $sql, 'WPO090 採番テーブル更新');
				...
				$this->execWithPDOLog($pdo, $sql, 'WPO090 トータルピッキングデータ採番更新');
				...
				$sql .= "   AND SYOMI_KIGEN='" . $result_totalpk['SYOMI_KIGEN'] . "';";
				$sql .= ";";
				$this->execWithPDOLog($pdo, $sql, 'WPO090 トータルピッキングiPodデータ作成');
			...
			$sql .= " WHERE NINUSI_CD = '" . $this->_ninusi_cd . "'";
			$sql .= "   AND SOSIKI_CD = '" . $this->_sosiki_cd . "'";
			$this->execWithPDOLog($pdo, $sql, 'WPO090  IF_商品マスタ削除');
	...
	function getCSV() {
		...
		try{
			...
			$this->printLog("fatal", "例外発生", "WPO090", $e->getMessage());
			$pdo = null;
		}
