...
class HRD020Model extends DCMSAppModel{

	public $name = 'HRD020Model';
	...
	*/
	...
	public function getJinzaiCsvData($staff_cd, $haken_kaisya_cd, $staff_nm, $del_flg) {
		...
		try{
			...
			// 発行するSQLを設定
			$sql  = "SELECT distinct M1.STAFF_CD";
			$sql .= "     ,M1.STAFF_NM";
			$sql .= "	  ,M3.KAISYA_NM ";
			...
			$sql .= " 	  ,M1.CHINGIN_MAX";
			$sql .= "    FROM M_STAFF_KIHON M1 LEFT JOIN M_MEISYO M2 ON (M1.KBN_PST=M2.MEI_CD AND M2.MEI_KBN='17' AND M1.DAI_NINUSI_CD=M2.NINUSI_CD AND  M1.DAI_SOSIKI_CD=M2.SOSIKI_CD) ";
			$sql .= "    LEFT JOIN M_HAKEN_KAISYA M3 ON (M1.HAKEN_KAISYA_CD=M3.KAISYA_CD AND M1.DAI_NINUSI_CD=M3.NINUSI_CD AND  M1.DAI_SOSIKI_CD=M3.SOSIKI_CD) ";
			$sql .= "    LEFT JOIN M_MEISYO M4 ON (M1.KBN_KEIYAKU=M4.MEI_CD AND M4.MEI_KBN='30' AND M1.DAI_NINUSI_CD=M4.NINUSI_CD AND  M1.DAI_SOSIKI_CD=M4.SOSIKI_CD)     ";
			$sql .= "    LEFT JOIN M_MEISYO M5 ON (M1.KBN_PICK=M5.MEI_CD AND M5.MEI_KBN='19' AND M1.DAI_NINUSI_CD=M5.NINUSI_CD AND  M1.DAI_SOSIKI_CD=M5.SOSIKI_CD)    ";
			$sql .= " WHERE ";
			$sql .= "  M1.DAI_NINUSI_CD='" . $this->_ninusi_cd . "' AND";
			$sql .= "  M1.DAI_SOSIKI_CD='" . $this->_sosiki_cd . "'";
			if ($staff_cd != "") {
			...
			$this->printLog("fatal", "例外発生", "HRD020", $e->getMessage());
	...
	public function getSyugyoCsvData($startYmd, $endYmd, $staff_cd, $haken_kaisya_cd, $staff_nm,$del_flg) {
		...
		try{
			...
			$sql .= "			,HOUR(TIMEDIFF(T1.DT_SYUGYO_ED,T1.DT_SYUGYO_ST)) + TRUNCATE(MINUTE(TIMEDIFF(T1.DT_SYUGYO_ED,T1.DT_SYUGYO_ST))/60 + 0.005,2) AS A14";
			$sql .= "			,HOUR(T1.DT_SYUGYO_ST) +";
			$sql .= "			 TRUNCATE(MINUTE(T1.DT_SYUGYO_ST)/60,2) AS A15";
			$sql .= "			,DATEDIFF(T1.DT_SYUGYO_ED,T1.DT_SYUGYO_ST) * 24 + HOUR(T1.DT_SYUGYO_ED) +";
			$sql .= "			 TRUNCATE(MINUTE(T1.DT_SYUGYO_ED)/60,2) AS A16";
			$sql .= "			,HOUR(TIMEDIFF(T1.DT_SYUGYO_ED,T1.DT_SYUGYO_ST)) + TRUNCATE(MINUTE(TIMEDIFF(T1.DT_SYUGYO_ED,T1.DT_SYUGYO_ST))/60 + 0.005,2) ";
			$sql .= "			- SUM(IFNULL(HOUR(TIMEDIFF(T2.DT_KOTEI_ED,T2.DT_KOTEI_ST)),0) +";
			$sql .= "			IFNULL(TRUNCATE(MINUTE(TIMEDIFF(T2.DT_KOTEI_ED,T2.DT_KOTEI_ST))/60 + 0.005,2),0)) AS A17";
			$sql .= "			,SUM(IFNULL(HOUR(TIMEDIFF(T2.DT_KOTEI_ED,T2.DT_KOTEI_ST)),0) +";
			$sql .= "			IFNULL(TRUNCATE(MINUTE(TIMEDIFF(T2.DT_KOTEI_ED,T2.DT_KOTEI_ST))/60 + 0.005,2),0)) AS A18";
			...
			$sql .= "					INNER JOIN M_HAKEN_KAISYA M3 ON (M1.HAKEN_KAISYA_CD=M3.KAISYA_CD AND M1.DAI_NINUSI_CD=M3.NINUSI_CD AND  M1.DAI_SOSIKI_CD=M3.SOSIKI_CD)";
			$sql .= " WHERE ";
			...
			$this->printLog("LOG", "就業実績CSV", "HRD020", $sql);
	...
	public function getKoteiCsvData($startYmd, $endYmd, $daibunruicd, $cyubunruicd, $saibunruicd) {
		...
		try{
			...
			$sql .= " WHERE ";
			$sql .= "  BAT.NINUSI_CD='" . $this->_ninusi_cd . "' AND";
			$sql .= "  BAT.SOSIKI_CD='" . $this->_sosiki_cd . "'";
			if ($startYmd != "") {
	...
	public function old_getSyugyoCsvData($startYmd, $endYmd, $staff_cd, $haken_kaisya_cd, $staff_nm,$del_flg) {
		...
		try{
			...
			$sql .= "			END AS A33";
			$sql .= "			,HOUR(T1.DT_SYUGYO_ST) +";
			$sql .= "			 TRUNCATE(MINUTE(T1.DT_SYUGYO_ST)/60,2) AS A34";
			$sql .= "			,DATEDIFF(T1.DT_SYUGYO_ED,T1.DT_SYUGYO_ST) * 24 + HOUR(T1.DT_SYUGYO_ED) +";
			$sql .= "			 TRUNCATE(MINUTE(T1.DT_SYUGYO_ED)/60,2) AS A35";
			$sql .= "			FROM (T_KOTEI_H T1 LEFT JOIN (";
			$sql .= "			SELECT DD.NINUSI_CD,DD.SOSIKI_CD,DD.STAFF_CD,DD.YMD_SYUGYO,DD.DT_KOTEI_ST,DD.DT_KOTEI_ED";
			$sql .= "			FROM T_KOTEI_D DD";
			$sql .= "			WHERE DD.BNRI_DAI_CD='" . $restCD  . "'";
			...
			$col71= "";	$col72= "";	$col73= "";	$col74= "";	$col_rest=0;
			$shinya_time = 0;
			$shinya_syugyo = 0;
			$columnCount = 1;
			$linecnt = 0;
			while ($entity = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				if ($col1 == $entity["A1"] && $col3 == $entity["A3"]) {
					if ($columnCount==1){
						...
						$col12 = $entity["A12"];
						$col13 = $entity["A13"];
						// 2016.08.30 K.Araishi Modify
						$col14 = $entity["A14"];
						//$col14 = $entity["A17"];
						$col15 = $entity["A15"];
						$col16 = $entity["A16"];

						//2016.09.02 K.Araishi Add
						$col73 = $entity["A34"];
						$col74 = $entity["A35"];
					...
					// 2016.08.30 K.Araishi Add
					//$col17 = $col17 - $col58;
					$col14 = $col74 - $col73;
					//$col17 = $col14 - $col20;
					$col17 = $col14 - $col_rest;
					...
					//$outarray = array($col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9,$col10,$col11,$col12,$col13,$col14,$col15,$col16,$col17);
					$outarray = array($col1,$col2,$col3,$col4,$col5,$col6,$col7,$col8,$col9,$col10,$col11,$col12,$col13,$col14,$col73,$col74,$col17,$col_rest);
