...
App::uses('HRD030Model', 'Model');

class HRD030Controller extends AppController {
	...
	public function beforeFilter() {
		...
		$this->HRD030Model = new HRD030Model($this->Session->read('select_ninusi_cd'),
                                     $this->Session->read('select_sosiki_cd'),
                                     $this->name
                                     );
	...
	function index(){
		...
		//派遣コンボ設定
		$arrayList = $this->HRD030Model->getHaken();
	...
	function csvjinzai() {
		...
		// 人材基本情報CSVデータを取得
		$csvData = null;
		$csvData = $this->HRD030Model->getJinzaiCsvData($staff_cd, $haken_kaisya_cd, $staff_nm, $del_flg);
	...
	function csvsyugyo() {
		...
		// 就業実績情報CSVデータを取得
		$csvData = null;
		$csvData = $this->HRD030Model->getSyugyoCsvData($startYmd, $endYmd, $staff_cd, $haken_kaisya_cd, $staff_nm,$del_flg);
		// 作成するファイル名の指定
	...
	function csvkotei() {
		...
		// 工程実績情報CSVデータを取得
		$csvData = null;
		$csvData = $this->HRD030Model->getKoteiCsvData($startYmd, $endYmd, $daibunruicd, $cyubunruicd, $saibunruicd);
