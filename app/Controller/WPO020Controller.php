...
App::uses('WPO020Model', 'Model');
App::uses('MMeisyo', 'Model');

class WPO020Controller extends AppController {
	...
	public function beforeFilter() {
		...
		// 機能名
		$this->set('kino', Sanitize::stripAll($this->Session->read('menu_kino')['WPO']['020']['kino_nm']));
		...
		//データベース
		$this->WPO020Model = new WPO020Model($this->Session->read('select_ninusi_cd'),$this->Session->read('select_sosiki_cd'), $this->name);
	...
	function index(){
		...
		if (isset($this->request->query['display']) && $display == "true") {
			...
			$this->Session->write('displayWPO020','true');
			
			$this->redirect('/WPO020/index?return_cd=' . $return_cd . 
										  '&message=' . $message .
			...
			return;
		} else if ($this->Session->check('displayWPO020') && $display == "false") {
			
			$this->Session->delete('displayWPO020');
			
		} else if (!$this->Session->check('displayWPO020') && $display == "false") {
			
			$this->redirect('/WPO020/index?batch_nm_cd=' . $batch_nm_cd . 
										  '&s_batch_no=' . $s_batch_no . 
										  '&s_kanri_no=' . $s_kanri_no . 
										  '&zone=' . $zone . 
										  '&staff_nm=' . $staff_nm . 
		...
		//データ取得
		$lsts = $this->WPO020Model->getSKanriLst($batch_nm_cd, $s_batch_no, $s_kanri_no, $zone, $staff_nm, $kbn_status);
	...
	function update(){
		...
		//Javascript設定
		$this->set("onload","initReload();" . 
							"initPopUp('".Router::url('/WPO020/popup', false)."');" .
							"initUpdate('".Router::url('/WPO020/dataUpdate', false)."');");
		...
		if (isset($this->request->query['display']) && $display == "true") {
			...
			$this->redirect('/WPO020/update?return_cd=' . $return_cd . 
										  '&message=' . $message .
										  '&batch_nm_cd=' . $batch_nm_cd . 
										  '&s_batch_no=' . $s_batch_no . 
										  '&s_kanri_no=' . $s_kanri_no . 
			...
			$this->redirect('/WPO020/update?batch_nm_cd=' . $batch_nm_cd . 
										  '&s_batch_no=' . $s_batch_no . 
										  '&s_kanri_no=' . $s_kanri_no . 
										  '&zone=' . $zone . 
										  '&staff_nm=' . $staff_nm . 
	...
	public function dataUpdate() {
		...
		//TPM040との排他確認
		if (!$this->WPO020Model->checkTPM040($this->Session->read('staff_cd'))) {
		
			//エラー吐き出し
			$array = $this->WPO020Model->errors;
		    $message = '';
		...
		//更新処理
		if (!$this->WPO020Model->setTANAKETSU($this->Session->read('staff_cd'),$data)) {
	...
	public function popup() {
		...
		//ステータス（状態区分 棚欠）取得
		$stsketus = $this->MMeisyo->getMeisyo(Configure::read('mei_kbn.sts'),
				                            array('11','19'),
				                            array(MMeisyo::mei_1),
				                            array(MMeisyo::val_free_num_1));
		
		//Viewにセット
		$this->set('stsketus',$stsketus);
	}	
}
