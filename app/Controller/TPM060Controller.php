...
App::uses('TPM060Model', 'Model');

class TPM060Controller extends AppController {
	...
	public function beforeFilter() {
		...
		// 機能名
		$this->set('kino', Sanitize::stripAll($this->Session->read('menu_kino')['TPM']['060']['kino_nm']));
		...
		$this->TPM060Model = new TPM060Model($this->Session->read('select_ninusi_cd'),
                                     $this->Session->read('select_sosiki_cd'),
                                     $this->name
                                     );
	...
	function index(){
		...
		$this->set("onload","initReload();".
					"initCSV('".Router::url('/TPM060/csv', false). "');" .
					"initBunrui();");
		...
		if ($ymd_from == '' && $ymd_to == '') {
			$ymd_from = $this->TPM060Model->getToday();
			$ymd_to = $ymd_from;
			$ninzu = 'on';
			$jikan = 'on';
		}
		...
		if (isset($this->request->query['display']) && $display == "true") {
			...
			$this->Session->write('displayTPM060','true');

			$this->redirect('/TPM060/index?return_cd=' . $return_cd .
										  '&message=' . $message .
										  '&ymd_from=' . $ymd_from .
										  '&ymd_to=' . $ymd_to .
										  '&kotei=' . $kotei .
			...
			return;
		} else if ($this->Session->check('displayTPM060') && $display == "false") {

			$this->Session->delete('displayTPM060');

		} else if (!$this->Session->check('displayTPM060') && $display == "false") {

			$this->redirect('/TPM060/index?&ymd_from=' . $ymd_from .
										  '&ymd_to=' . $ymd_to .
										  '&kotei=' . $kotei .
										  '&kotei2=' . $kotei2 .
										  '&timeFrom=' . $timeFrom .
		...
		$lsts = array();
		$timeArray = $this->TPM060Model->getTimes();
		...
		//検索条件の必須チェック
		if (!$this->Session->check('displayTPM0602') && isset($this->request->query['ymd_from']) &&
				($ymd_from == '' || $ymd_to == '') && !isset($this->request->query['return_cd']) ) {
					...
					$this->Session->write('displayTPM0602','true');

					$this->redirect('/TPM060/index?return_cd=1' .
										  '&message=' . $message .
										  '&ymd_from=' . $ymd_from .
										  '&ymd_to=' . $ymd_to .
										  '&kotei=' . $kotei .
				...
				$this->Session->delete('displayTPM0602');
		}
		...
		if (($ymd_from != '' && $ymd_to != '') || $kotei != '' || $timeFrom || $timeTo) {
			...
			//リスト一覧
			$lsts =  $this->TPM060Model->getKoteiList($ymd_from, $ymd_to, $kotei, $timeArray, $ninzu, $jikan, $kotei2);
			...
			if ($this->Session->check('saveTableTPM060')) {

				//前回のデータを取得
				$getArray = $this->Session->read('saveTableTPM060');
				...
				//前回のデータを削除
				$this->Session->delete('saveTableTPM060');

				//エラーが発生していない場合は、検索条件を元に表示
			} else {

				//タイムスタムプ取得
				$this->set("timestamp", $this->TPM060Model->getTimestamp());
	...
	public function csv() {
		...
		// 工程実績情報CSVデータを取得
		$csvData = $this->TPM060Model->getCSV($YMD_FROM, $YMD_TO, $KOTEI, $timeArray, $ninzu, $jikan, $kotei2);
