...
App::uses('WPO080Model', 'Model');

class WPO080Controller extends AppController {
	...
	public function beforeFilter() {
		...
		//WPO080モデル
		$this->WPO080Model = new WPO080Model($this->Session->read('select_ninusi_cd'),$this->Session->read('select_sosiki_cd'),$this->name);
	...
	function index(){
		...
		//Javascript設定
		$this->set("onload",
							"initCSV('".Router::url('/WPO080/CSV', false)."');" .
							"initUpdate('".Router::url('/WPO080/updateData', false)."');" .
							"initImport('".Router::url('/WPO080/importData', false)."');");
		...
		$shukkaLst = $this->WPO080Model->loadIfShukka();
		$checkLst = $this->WPO080Model->loadCheckIfShukka();
	...
	public function importData() {
		...
		// 登録
		$this->WPO080Model->InsertIfShukka($records, $staffCd);

		$array = $this->WPO080Model->errors;
		if (!empty($array)) {
		    $message = '';
	...
	public function updateData() {
		...
		if (!$this->WPO080Model->checkIfShukka()) {
			$array = $this->WPO080Model->errors;
			if (!empty($array)) {
			    $message = '';
			    foreach ( $array as $key=>$val ) {
		...
		// 更新処理
		$this->WPO080Model->updateShukka($staffCd);
	...
	public function csv() {
		...
		$csvData = $this->WPO080Model->getCSV();

		// 作成するファイル名の指定
		$fileName = "ＩＦ出荷指示取込情報_" . date("Ymdhi") . ".csv";
		$buff = '';
