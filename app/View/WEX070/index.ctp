			...
			<tr>
				<td>
					...
					<div style="position:relative;width:610px;overflow:hidden;float:left;margin-top:3px">
						<?php echo $this->Form->file('file_name', array('id' => 'refText', 'size' => '150', 'name' => 'csv')) ?>
					</div>
				...
				<td class="colBnriCd" style="text-align:left;">
				    	<div style="width:100px;float:left;margin-top:10px;text-align:left;">
					    	<input name="start_ymd_ins" id="startYmdInsText" maxlength="8" class="calendar han" type="text" style="width:80px;height:20px" 
					    		value="<?php if(!empty($ymd_from)) echo str_replace("-","",$ymd_from); ?>">
		    			</div>
					...
					　～　
				    	...
				    	<div style="width:100px;float:left;margin-top:10px;text-align:left;">
					    	<input name="end_ymd_ins" id="endYmdInsText" maxlength="8" class="calendar han" type="text" style="width:80px;height:20px" 
					    		value="<?php if(!empty($ymd_to)) echo str_replace("-","",$ymd_to); ?>">
		    			</div>
				</td>
			</tr>
			<!-- ２行目 -->
...
?>
	...
	<div class="tableRow row_dat" style="width:1740px;">
		<div class="tableCellBango cell_dat bango" style="width:60px;"><?php echo ($index + ($count + 1)) ?></div>
		<div class="tableCell cell_dat date" style="width:90px;"><?php echo date_format(date_create($array['YMD']),"Y/m/d") ?></div>
		...
		<div class="tableCell cell_dat kbn" style="width:60px;"><?php echo $array['KBN_TANI_NM'] ?></div>
		<div class="tableCell cell_dat nm" style="width:100px;"><?php echo $array['KBN_KTD_MKTK_NM'] ?></div>
		<div class="tableCell cell_dat kin" style="width:90px;"><?php echo number_format($array['TANKA']) ?></div>
		<div class="tableCell cell_dat cnt" style="width:80px;"><?php echo number_format($array['JIKAN'],2) ?></div>
		<div class="tableCell cell_dat cnt" style="width:60px;"><?php echo number_format($array['NINZU']) ?></div>
		<div class="tableCell cell_dat cnt" style="width:80px;"><?php
		
		if ($array['KBN_URIAGE'] != "01") {
			echo number_format($array['BUTURYO']);
		} else {
			echo $array['KBN_URIAGE_NM'];
		}
		...
		if ($array['KBN_URIAGE'] != "01") {
			echo number_format($array['SURYO_LINE']);
		} else {
			echo $array['KBN_URIAGE_NM'];
		}
		
		?></div>
		<div class="tableCell cell_dat cnt" style="width:100px;"><?php echo number_format($array['ABC_COST']) ?></div>
		<div class="tableCell cell_dat kin" style="width:100px;"><?php echo number_format($array['URIAGE']) ?></div>
		<div class="tableCell cell_dat kin" style="width:100px;"><?php echo number_format($array['RIEKI']) ?></div>
		<div class="tableCell cell_dat cnt" style="width:100px;" ><?php echo number_format($array['SEISANSEI_BUTURYO'],1); ?></div>
		<div class="tableCell cell_dat cnt" style="width:100px;" ><?php echo number_format($array['SEISANSEI_SURYO_LINE'],1); ?></div>
		<div class="tableCell cell_dat kbn" style="width:60px;"><?php echo $array['KBN_GET_DATA_NM'] ?></div>
		...
		<div class="hiddenData"><?php echo $array['YMD'] ?></div>
		<div class="hiddenData"><?php echo $array['BUTURYO'] ?></div>
		<div class="hiddenData"><?php echo $array['SURYO_LINE'] ?></div>
		<div class="hiddenData"><?php echo $array['KOTEI_KBN'] ?></div>
		<div class="hiddenData"><?php echo $array['KBN_URIAGE'] ?></div>
		<div class="hiddenData"><?php echo $array['KBN_URIAGE_NM'] ?></div>
	</div>
